//
// Created by samariddin on 25.11.2019.
//
...
int main() {
    ...
    long long firstEncStep = encAndDec(text, ca, na);
    long long textEncForBob = encAndDec(firstEncStep, db, nb);

    long long firstDecStep = encAndDec(textEncForBob, cb, nb);
    long long textDecBob = encAndDec(firstDecStep, da, na);
